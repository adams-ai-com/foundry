...
export default async function CallPage({ params }: Props) {
  ...
  const [call] = await db`
    SELECT id, title, status, channel_id, topic_id, created_by_name, livekit_room_name
    FROM video_calls
    WHERE id = ${callId} AND org_id = ${session.orgId}
  ` as unknown as [{
    id: string; title: string; status: string
    channel_id: string | null; topic_id: string | null
    created_by_name: string; livekit_room_name: string
  }?]
      ...
      <div className="min-h-screen bg-neutral-950 flex items-center justify-center p-4">
        <div className="text-center">
          <div className="w-12 h-12 bg-white/5 rounded-full flex items-center justify-center mx-auto mb-4">
            <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-white/40">
