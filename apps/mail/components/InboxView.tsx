...
export const InboxView = forwardRef<InboxViewHandle, InboxViewProps>(
  function InboxView({ mailbox, selectedThread, onSelectThread }, ref) {
    ...
    if (prevKeyRef.current !== key) { prevKeyRef.current = key; if (page !== 1) setPage(1) }

    const load = useCallback(async () => {
      if (search.trim()) return
      setLoading(true)
      try {
        if (filter === 'unread') { const rows = await searchThreads('is:unread'); setThreads(rows); setTotal(rows.length) }
        else if (filter === 'starred') { const rows = await searchThreads('is:starred'); setThreads(rows); setTotal(rows.length) }
        else { const { threads: rows, total: t } = await listThreads(mailbox, page, sort); setThreads(rows); setTotal(t) }
    ...
    useImperativeHandle(ref, () => ({
      reload: load,
      removeThread: (id) => setThreads((p) => p.filter((t) => t.id !== id)),
      getThreads: () => threads,
    }), [load, threads])
    ...
    const handleSearch = async (q: string) => {
      ...
      setSearching(true)
      try { const rows = await searchThreads(q.trim()); setThreads(rows); setTotal(rows.length) }
      finally { setSearching(false) }
    }

    const handleAction = async (e: React.MouseEvent, thread: MailThread, action: 'star' | 'archive' | 'trash') => {
      e.stopPropagation()
      try {
        if (action === 'star') { await starThread(thread.id, !thread.isStarred); setThreads((p) => p.map((t) => t.id === thread.id ? { ...t, isStarred: !t.isStarred } : t)) }
        else if (action === 'archive') { await archiveThread(thread.id); setThreads((p) => p.filter((t) => t.id !== thread.id)) }
        else if (action === 'trash') { await trashThread(thread.id); setThreads((p) => p.filter((t) => t.id !== thread.id)) }
      } catch { /* ignore */ }
    }
    ...
    return (
      <div className="w-80 border-r border-border flex flex-col flex-shrink-0 bg-bg-surface">
        ...
        <div className="flex border-b border-border px-3 bg-bg-raised">
          {(['all', 'unread', 'starred'] as const).map((f) => (
            <button key={f} onClick={() => { setFilter(f); setSearch('') }}
              className={`text-xs px-3 py-2 capitalize transition-colors ${filter === f ? 'border-b-2 border-accent text-accent font-medium' : 'text-fg-secondary hover:text-fg-primary'}`}>
              {f}
            </button>
          ))}
        </div>
        ...
        <div className="px-3 py-2 border-b border-border flex gap-2 items-center bg-bg-raised">
          ...
          {!inSearch && !inFilter && (
            <select value={sort} onChange={(e) => setSort(e.target.value as 'newest' | 'oldest' | 'unread')}
        ...
        <div className="flex-1 overflow-y-auto">
          {(loading || searching) && <div className="flex items-center justify-center py-8 text-fg-tertiary text-sm">{searching ? 'Searching…' : 'Loading…'}</div>}
          {!loading && !searching && threads.length === 0 && <div className="flex items-center justify-center py-8 text-fg-tertiary text-sm">{inSearch ? 'No results' : 'No messages'}</div>}
          {!loading && !searching && threads.map((thread) => {
            ...
            return (
              <div key={thread.id} onClick={() => onSelectThread(thread)}
                className={`group relative border-b border-border cursor-pointer transition-colors ${isSelected ? 'bg-bg-active' : 'hover:bg-bg-hover'}`}>
                <div className="px-4 py-3 pr-2">
                  <div className="flex items-center justify-between mb-1">
                    <span className={`text-sm overflow-hidden text-ellipsis whitespace-nowrap max-w-[140px] ${isUnread ? 'font-semibold text-fg-primary' : 'text-fg-secondary'}`}>{senderLabel}</span>
                    <span className="text-xs text-fg-tertiary flex-shrink-0 ml-2 tabular-nums">{formatDate(thread.lastMessageAt)}</span>
                  </div>
                  <div className={`text-sm overflow-hidden text-ellipsis whitespace-nowrap ${isUnread ? 'font-medium text-fg-primary' : 'text-fg-secondary'}`}>{thread.subject}</div>
                  <div className="text-xs text-fg-tertiary overflow-hidden text-ellipsis whitespace-nowrap mt-0.5">{thread.snippet}</div>
                </div>
                {isUnread && <div className="absolute left-1.5 top-4 w-1.5 h-1.5 rounded-full bg-accent" />}
                <div className="absolute right-1 top-1/2 -translate-y-1/2 hidden group-hover:flex items-center gap-0.5 bg-bg-raised rounded-lg shadow-card border border-border px-1 py-1">
                  <button onClick={(e) => handleAction(e, thread, 'star')} title={thread.isStarred ? 'Unstar' : 'Star'}
                    className={`p-1 rounded hover:bg-bg-hover transition-colors ${thread.isStarred ? 'text-amber-400' : 'text-fg-tertiary hover:text-amber-400'}`}>
                    {thread.isStarred ? <StarFilledIcon className="w-3.5 h-3.5" /> : <StarIcon className="w-3.5 h-3.5" />}
                  </button>
                  <button onClick={(e) => handleAction(e, thread, 'archive')} title="Archive (e)"
                    className="p-1 rounded hover:bg-bg-hover transition-colors text-fg-tertiary hover:text-fg-primary">
                    <ArchiveIcon className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={(e) => handleAction(e, thread, 'trash')} title="Trash (#)"
