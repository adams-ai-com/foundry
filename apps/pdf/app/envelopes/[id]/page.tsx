...
export default function EnvelopePage() {
  ...
  async function downloadCertificate() {
    setDownloading(true)
    try {
      const res = await fetch(`/pdf/api/envelopes/${id}/certificate`)
      if (!res.ok) { alert('Certificate generation failed'); return }
      const blob = await res.blob()
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url; a.download = `completion-certificate-${id.slice(0, 8)}.pdf`; a.click()
        ...
        {envelope.status === 'complete' && (
          <div className="mt-4">
            <button onClick={downloadCertificate} disabled={downloading}
              className="flex items-center gap-1.5 text-xs font-medium text-fg-secondary border border-border rounded-lg px-3 py-1.5 hover:bg-bg-hover disabled:opacity-40">
