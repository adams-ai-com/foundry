...
export async function listThreads(
  ...
  const [threads, countRows] = await Promise.all([
    sql<any[]>`
      ...
      WHERE t.account_id = ${accountId} AND m.mailbox_id = ${mailboxId}
...
export async function getThread(accountId: string, threadId: string): Promise<ThreadDetail | null> {
  ...
  const messages = await sql<any[]>`
    SELECT m.id, m.from_email, m.from_name, m.to_addrs, m.cc_addrs,
...
export async function searchThreads(accountId: string, query: string, limit = 20): Promise<ThreadRow[]> {
  ...
  const rows = await sql<any[]>`
    ...
    WHERE t.account_id = ${accountId}
      AND (
        m.search_vector @@ to_tsquery('english', ${tsQuery})
        OR t.subject ILIKE ${'%' + query + '%'}
        OR m.from_email ILIKE ${'%' + query + '%'}
      )
